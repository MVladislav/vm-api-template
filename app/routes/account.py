...
from app.persist.account.schemas.response import (ResponseLoginSchema,
                                                  ResponseRegistrationSchema,
                                                  ResponseSchema)
...
@router.post("/login", name="account:login",
...
async def login(items: RequestLoginSchema, response: Response) -> Any:
    ...
    """
    return await requestHelper(response=response, func=account.login, keys=items,
                               keysNeeded=RequestLoginSchema, funcCallerName="login")


@router.post("/register", name="account:register",
             response_model=ResponseRegistrationSchema, status_code=status.HTTP_201_CREATED)
async def register(items: RequestRegistrationSchema, response: Response) -> Any:
    ...
    """
    return await requestHelper(response=response, func=account.registration, keys=items,
                               keysNeeded=RequestRegistrationSchema, funcCallerName="register")


@router.delete("/remove", name="account:remove",
               response_model=ResponseSchema)
async def remove(response: Response, jwt: TokenDataObject = Depends(middlewareAuth)) -> Any:
    """
        removes a existing user
    """
    return await requestHelper(token=jwt, response=response, func=account.remove, keys=None, keysNeeded=None, funcCallerName="remove")
